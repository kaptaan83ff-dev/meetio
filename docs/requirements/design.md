...
The atmosphere is one of "Raw Precision." It feels like a high-end terminal interface or a physical ledger book brought to life. Key motifs include **dot-grid overlays** (reminiscent of architectural blueprints), **browser-style window chrome** (three-dot window controls), and **mono-weighted iconography**. The depth is entirely functional — elements don't "glow" or "float"; they are "lifted" or "pressed" onto the canvas with fixed mathematical offsets. It is serious enough to represent AI engineering, but bold enough to feel like a boutique creative studio.
...
- **Ledger Motif:** Use of window-like frames (LEDGER V12) with terminal-inspired status indicators.
- **Utility Typography:** Heavy display faces (Archivo Black) paired with geometric sans-serifs (Inter) for a "technical-yet-bold" voice.
...
- **Electric Pink** (`#ff4f8b`): The secondary highlight. Used for specific division headers (e.g., "HOW WE PRODUCE") and as a counter-accent to the yellow.
- **Neon Lime** (`#c6ff3d`): Used as a secondary accent for success states, specific tags, and "active" status dots.
- **Royal Blue** (`#0052ff`): Reserved for specific utility cards (e.g., "Multi-GPU Cluster") to break the warm palette and signal "technical" infrastructure.
...
- **Ghost White** (`#ffffff`): Used inside cards and window frames to differentiate "content areas" from the "canvas."
...
- **Inverted Yellow** (`#ffe500`): Used for text on dark backgrounds (e.g., black buttons).
- **Muted Metadata** (`rgba(10, 10, 10, 0.6)`): Used for secondary labels, "division" counts, and micro-text.

## 3. Typography Rules

### Font Family

- **Archivo Black**: The signature display font. Used for the wordmark, massive hero headlines, and button labels. It is heavy, condensed, and carries the "Brutalist" weight.
- **Inter**: The UI and body workhorse. Used for descriptive text, secondary labels, and terminal-style content. Provides legibility and a modern "tech" feel.
- **Monospace (System)**: Used sparingly for version numbers (e.g., "V12") and terminal-style status text.
...
| Card Title     | Archivo Black | 24px / 1.5rem  | 900    | 1.10        | -0.5px         | Component and module headlines         |
| Nav Link       | Archivo Black | 12px / 0.75rem | 400    | 1.00        | 0.6px          | UPPERCASE, tracking-wider              |
| Body Text      | Inter         | 20px / 1.25rem | 500    | 1.55        | 0px            | Primary reading content                |
| Small Label    | Archivo Black | 12px / 0.75rem | 400    | 1.20        | 0.8px          | UPPERCASE, used for "DIVISION 01/04"   |
| Tag / Badge    | Inter         | 11px / 0.69rem | 700    | 1.00        | 0.5px          | UPPERCASE, inside small pills          |

### Principles

- **Weight is Authority:** Use Archivo Black for almost all UI triggers (buttons, nav, headers). Inter is reserved purely for "information."
- **Tight Leading on Headlines:** Huge headlines (120px) must use a line-height of `0.9` to create a dense, "blocky" visual impact.
- **UPPERCASE Everything for UI:** Nav links, button text, and category labels are almost exclusively uppercase with wide tracking (`0.6px` to `0.8px`).
...
**The "Ledger" Window (Content Cards)**

- **Container:** `3px solid #0a0a0a` border.
- **Header Bar:** 32px height, solid background (matching section color or white).
- **Chrome:** Three status dots on the left (`#ff4f8b`, `#ffe500`, `#c6ff3d`).
...
- **Visual Kicker:** "DIVISION 01 / 04" style labels pinned to the top-left of sections.
...
- **Logo:** "MeetIO" in Archivo Black 16px, often accompanied by a custom "Microchip" icon in a square border.
...
- **Section Padding:** Large 80px or 120px vertical gaps between major content blocks to allow the bold typography to "breathe."
...
Whitespace is not "empty" in this system; it is often filled with a **dot-grid texture**. This prevents the site from feeling sparse or "minimalist" and instead makes it feel like a "populated workspace." Content is tightly packed within cards, but the cards themselves are given generous margins on the canvas.
...
| 2     | `2px solid #0a0a0a` + `4px shadow` | Default state for buttons and small cards         |
| 3     | `3px solid #0a0a0a` + `4px shadow` | Main content "Ledger" windows                     |
| 4     | `2px solid #0a0a0a` + `7px shadow` | Hover state (element "lifts" off the page)        |
| 5     | `2px solid #0a0a0a` + `1px shadow` | Active/Pressed state (element "pushed" into page) |
...
- **Do** wrap primary content in "Ledger" window frames with the three-dot chrome.
...
- **Don't** use Inter for headlines. It is a utility font only.
...
- **Tablet:** `768px - 1023px`. 2-column grid for cards. Hero text scales to 80px. Nav collapses to hamburger menu.
...
- **Secondary:** `#ff4f8b` (Pink)
- **Accent:** `#c6ff3d` (Lime)

### Example Component Prompts

1. _"Create a Neo-Brutalist primary button: Background `#ffe500`, 2px solid `#0a0a0a` border, 4px 4px 0px 0px `#0a0a0a` hard shadow. Text is 'WORK WITH US' in 12px Archivo Black UPPERCASE. On hover, translate by -2px -2px and increase shadow to 7px."_
